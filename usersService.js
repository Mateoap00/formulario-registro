...
const getUsers = () => {
    return fetch('http://localhost:9001/list-users.php')
...
const addUser = (user) => {
    fetch('http://localhost:9001/insert.php', { method: 'POST', body: JSON.stringify(user) })
        .then(response => response.json())
...
const updateUser = (user) => {
    fetch('http://localhost:9001/update.php', { method: 'post', body: JSON.stringify(user) })
        .then(response => response.json())
        .then(data => console.log(data))
        .catch((error) => {
            console.error(error);
...
// de la base de datos 'store'.
const deleteUser = (id) => {
    fetch(`http://localhost:9001/delete.php?id=${id}`)
        .then(response => response.json())
        .then(data => console.log(data))
        .catch((error) => {
            console.error(error);
